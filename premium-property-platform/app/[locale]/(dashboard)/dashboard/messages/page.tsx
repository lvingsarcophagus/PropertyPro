...
import { useI18n } from '@/lib/i18n/index'; // Import useI18n for client components
...
export default function MessagesPage() {
  ...
  const { t } = useI18n(); // Initialize translations for client component
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      if (usersError) {
        ...
        setError(t('messages.error_loading_users'));
        setAllUsers([]);
      } else {
        setAllUsers(data || []);
      }
  ...
  const fetchMessages = useCallback(async (partnerId: string) => {
    ...
    if (messagesError) {
      ...
      setError(t('messages.error_loading_conversation'));
      setMessages([]);
    } else {
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    const optimisticSenderName = currentUser.user_metadata?.name || currentUser.email?.split('@')[0] || t('messages.you');
    ...
    if (sendError) {
      ...
      setError(t('messages.error_sending_message'));
  ...
  if (!currentUser && !isLoadingUsers) {
    return <div className="p-6 text-center text-slate-600">{t('messages.login_prompt')}</div>;
  }
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)]"> {/* Adjusted height for fixed navbar */}
      <div className="w-1/3 border-r border-slate-200 bg-slate-50 flex flex-col">
        <div className="p-4 border-b border-slate-200">
          <h2 className="text-xl font-semibold text-slate-800">{t('messages.conversations_title')}</h2>
        ...
        <div className="overflow-y-auto flex-grow">
          {isLoadingUsers ? (
            ...
            <p className="p-4 text-sm text-slate-500">{t('messages.no_users_found')}</p>
          )}
        </div>
      </div>
      ...
      <div className="w-2/3 flex flex-col bg-white">
        {selectedUserId ? (
          <>
            <div className="p-4 border-b border-slate-200 bg-slate-50">
              <h3 className="text-lg font-semibold text-slate-800">
                {t('messages.chat_with')} {allUsers.find(u => u.id === selectedUserId)?.name || allUsers.find(u => u.id === selectedUserId)?.email?.split('@')[0] || t('messages.selected_user')}
              </h3>
            </div>
            <div className="flex-grow overflow-y-auto p-4 space-y-3 bg-slate-100">
              {isLoadingMessages ? (
                <p className="text-sm text-slate-500 text-center">{t('messages.loading_messages')}</p>
              ) : messages.length > 0 ? (
                messages.map(msg => (
          ...
          <div className="flex-grow flex flex-col items-center justify-center text-slate-500 bg-slate-50 p-4">
            ...
            <p className="text-lg">{t('messages.select_conversation_prompt')}</p>
            <p className="text-sm">{t('messages.search_user_prompt')}</p>
          </div>
        )}
        {error && (
